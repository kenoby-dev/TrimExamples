...
```
  ...
  {
    return GetLocations().First(x => x.NameString.Equals(location, StringComparison.OrdinalIgnoreCase));
  }
```
...
```
  public Record GetRecord(string recordNr)
  {
    ...
    TrimSearchClause numberSearch = new TrimSearchClause(BaseObjectTypes.Record, TrimDb, SearchClauseIds.RecordNumber);
